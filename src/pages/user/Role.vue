...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "新权限" : "编辑权限";
    },
    dateCol() {
    ...
    tableDataFiltered() {
      ...
      return this.tableDataProcessed.filter(dateRangeFilter).map((el) => {
        ...
        this.$http
          ...
          .then((res) => {
            if (this.$http.hasError(res)) {
              this.editedItem.roleName = "权限不存在！";
  ...
  methods: {
    ...
    afterDeleteItemConfirm(item) {
      ...
      this.$http
        ...
        .then((res) => {
          if (this.$http.hasError(res)) {
            this.$message.error("删除失败！该权限不存在或无法删除！");
          } else {
    ...
    async saveItem(item) {
      ...
      try {
        let res = await this.$http.post("/user/role/saveRole", {
          role: item,
        });
        if (this.$http.hasError(res)) {
          ...
          throw new Error("权限" + msg + "权限保存失败！");
        } else {
          if (!item.roleId) {
            item.roleId = res.data.result.roleId;
